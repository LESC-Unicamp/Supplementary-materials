...
!                                        of the box matrix                                        !
! *********************************************************************************************** !
SUBROUTINE UndoBoxRotation( CurrentBoxLength, CurrentBoxLengthInverse, CurrentBoxVolume )

! Uses three modules: global variables, vector operations, and quaternion operations
USE GlobalVariables
...
REAL( Kind= Real64 ) :: CurrentBoxVolume      ! Box volume (NPT Simulation)

! REAL VARIABLES (ARRAY)
REAL( Kind= Real64 ), DIMENSION( 9 )               :: CurrentBoxLength            ! Box length (NPT Simulation)
REAL( Kind= Real64 ), DIMENSION( 9 )               :: CurrentBoxLengthInverse     ! Inverse of box length (NPT Simulation)
...
! Initialization
OldBoxLength = CurrentBoxLength
OldBoxLengthInverse = CurrentBoxLengthInverse

! Box vectors
BoxVectorX = CurrentBoxLength(1:3)
BoxVectorY = CurrentBoxLength(4:6)
BoxVectorZ = CurrentBoxLength(7:9)
...
IF( DABS( RotationAxisMagnitude - 0.D0 ) >= EPSILON( 1.D0 ) ) THEN
  ...
  BoxLengthBoxRotation(1:3) = DSQRT( DOT_PRODUCT( CurrentBoxLength(1:3), CurrentBoxLength(1:3) ) ) * RotatedVector
  ...
  BoxLengthBoxRotation(4:6) = DSQRT( DOT_PRODUCT( CurrentBoxLength(4:6), CurrentBoxLength(4:6) ) ) * RotatedVector
  ! Rotated z-vector
  ...
  BoxLengthBoxRotation(7:9) = DSQRT( DOT_PRODUCT( CurrentBoxLength(7:9), CurrentBoxLength(7:9) ) ) * RotatedVector
...
ELSE
  ...
  BoxLengthBoxRotation = CurrentBoxLength
  BoxLengthInverseBoxRotation = CurrentBoxLengthInverse
  BoxVolumeBoxRotation = CurrentBoxVolume
...
CurrentBoxLength = BoxLengthBoxRotation
CurrentBoxLengthInverse = BoxLengthInverseBoxRotation
CurrentBoxVolume = BoxVolumeBoxRotation
