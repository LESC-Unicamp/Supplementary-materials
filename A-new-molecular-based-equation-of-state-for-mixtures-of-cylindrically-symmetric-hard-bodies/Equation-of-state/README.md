...
| Non-spherical<br>mixing rule | <code>NON_SPHERICAL_MIXING_RULE</code> | Used to select the mixing rule for the non-spherical contribution | <ul><li><code>1</code> for the aspect-ratio mixing rule</li><li><code>2</code> for the angle-averaged excluded-volume mixing rule</li><li><code>3</code> for the ratio-of-second-virial-coefficients mixing rule</li><li><code>4</code> for the second-virial-coefficient mixing rule</li></ul> |
...
| Higher-order TPT | <code>HIGH_ORDER_TPT</code> | Used to include or neglect perturbation terms above second order | <ul><li><code>.TRUE.</code> to include higher-order terms</li><li><code>.FALSE.</code> to truncate the perturbation expansion at second order</li></ul> |
| Highest TPT order<br>(<code>HIGH_ORDER_TPT=.TRUE.</code>) | <code>N_HIGH_ORDER_TPT</code> | Used to define the highest perturbation order included in the expansion | Any <code>INTEGER</code> number greater than <code>2</code> |
| Effective packing-fraction mixing rule | <code>EFFECTIVE_PFRACTION_MIXING_RULE</code> | Used to select the mixing rule for the effective packing fraction | <ul><li><code>1</code> for the overall reduced-density mixing rule</li><li><code>2</code> for the one-fluid van der Waals mixing rule</li></ul> |
...
| Override second-order coefficients<br>(CSW) | <code>OVERWRITE_A2_COEFFS</code> | Used to replace the effective-packing-fraction coefficients of the second-order CSW perturbation term with those of the first-order term | <ul><li><code>.TRUE.</code> to use the first-order coefficients also for the second-order term</li><li><code>.FALSE.</code> to retain the second-order coefficients</li></ul> |
| Reference term | <code>BOUBLIK_REFERENCE</code> | Used to select the hard-convex-body reference contribution | <ul><li><code>.TRUE.</code> for the Boublik reference term</li><li><code>.FALSE.</code> for the Parsons-Lee reference term</li></ul> |
| All-geometry first-order coefficients<br>(CSW) | <code>OVERWRITE_A1_ALLGEO</code> | Used to replace the geometry-specific first-order CSW effective-packing-fraction coefficients with the coefficient set fitted using all available geometries | <ul><li><code>.TRUE.</code> to use the all-geometry coefficients</li><li><code>.FALSE.</code> to retain the geometry-specific coefficients</li></ul> |
| All-geometry second-order coefficients<br>(CSW) | <code>OVERWRITE_A2_ALLGEO</code> | Used to replace the geometry-specific second-order CSW effective-packing-fraction coefficients with the coefficient set fitted using all available geometries | <ul><li><code>.TRUE.</code> to use the all-geometry coefficients</li><li><code>.FALSE.</code> to retain the geometry-specific coefficients</li></ul> |
...
<p align="justify">
  <b>OBS. II</b>: The non-spherical mixing rules <code>1</code> and <code>2</code> require all components to have the same molecular geometry. Mixing rules <code>3</code> and <code>4</code> can be used when different geometries are present in the same mixture.
</p>
