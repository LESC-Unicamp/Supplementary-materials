...
!               This code contains the overlap algorithm used in the main program.                !
!                                                                                                 !
...
! *********************************************************************************************** !
SUBROUTINE OverlapCheckSPC( iOrientation, jOrientation, VectorDistance, SquaredDistance, ContactDistance, ParallelSPC )
...
REAL( Kind= Real64 )                   :: Theta2                              ! Cardano's parameters
REAL( Kind= Real64 )                   :: CosineArgument                     ! Cardano's parameters
...
IF( Alpha > 0.D0 ) THEN
  ...
  IF( Discriminant < 0.D0 ) THEN
    ...
    ELSE IF( CosineArgument >= 1.D0 ) THEN
      CosineArgument = 1.D0
    END IF
    Theta2 = DACOS( CosineArgument )
    ! Roots of the cubic equation
    CubicRoots(1) = 2.D0 * DSQRT( - CardanoQ ) * DCOS( (Theta2 / 3.D0) ) - cCubic(2) / ( 3.D0 * cCubic(1) )
    CubicRoots(2) = 2.D0 * DSQRT( - CardanoQ ) * DCOS( (Theta2 / 3.D0) + (2.D0 * cPi / 3.D0) ) - cCubic(2) / &
    &               ( 3.D0 * cCubic(1) )
    CubicRoots(3) = 2.D0 * DSQRT( - CardanoQ ) * DCOS( (Theta2 / 3.D0) + (4.D0 * cPi / 3.D0) ) - cCubic(2) / &
    &               ( 3.D0 * cCubic(1) )
