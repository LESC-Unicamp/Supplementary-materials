...
INTEGER( Kind= Int64 ) :: Image
INTEGER( Kind= Int64 ) :: Cylinder
...
REAL( Kind= Real64 ), DIMENSION( 3, 4, nParticles )          :: cPositionBoxRotation        ! Position of the center of mass of cylinders (after undoing box rotation)
REAL( Kind= Real64 ), DIMENSION( 3, nImages, nParticles )    :: imPositionBoxRotation       ! Position of the center of mass of images (after undoing box rotation)
REAL( Kind= Real64 ), DIMENSION( 3, nImages, nParticles )    :: imOrientationBoxRotation    ! Orientation of the center of mass of images (after undoing box rotation)
REAL( Kind= Real64 ), DIMENSION( 0:3, nImages, nParticles )  :: imQuaternionBoxRotation     ! Quaternion of the center of mass of images (after undoing box rotation)
REAL( Kind= Real64 ), DIMENSION( 3, 4, nImages, nParticles ) :: imcPositionBoxRotation      ! Position of the center of mass of cylindrical images (after undoing box rotation)
...
IF( DABS( RotationAxisMagnitude - 0.D0 ) >= EPSILON( 1.D0 ) ) THEN
  ...
  DO Particle = 1, nParticles
    ...
    CALL MatrixVectorMultiplication( BoxLengthInverseBoxRotation, pPositionBoxRotation(:,Particle), ScalingDistanceUnitBox )
    ...
    DO Image = 1, nImages
      CALL MatrixVectorMultiplication( BoxLengthBoxRotation, ScalingDistanceImageUnitBox(:,Image), &
      &                                imPositionBoxRotation(:,Image,Particle) )
    END DO
    ! Reorient particles
    ...
    ! Reorient images
    DO Image = 1, nImages
      imQuaternionBoxRotation(:,Image,Particle)  = pQuaternionBoxRotation(:,Particle)
      imOrientationBoxRotation(:,Image,Particle) = pOrientationBoxRotation(:,Particle)
    END DO
    ! Cylinders
    DO Cylinder = 1, 4
      ! Active transformation (translation)
      CALL ActiveTransformation( cReferencePosition(:,Cylinder), pQuaternionBoxRotation(:,Particle), cRotatedPosition(:,Cylinder) )
      ! Position of cylinders of a particle
      cPositionBoxRotation(:,Cylinder,Particle) = pPositionBoxRotation(:,Particle) + cRotatedPosition(:,Cylinder)
      ! Position of cylinder images of a particle
      CALL MatrixVectorMultiplication( BoxLengthInverseBoxRotation, cPositionBoxRotation(:,Cylinder,Particle), &
      ...
      DO Image = 1, nImages
        CALL MatrixVectorMultiplication( BoxLengthBoxRotation, ScalingDistanceImageUnitBox(:,Image), &
        &                                imcPositionBoxRotation(:,Cylinder,Image,Particle) )
      END DO
    END DO
  END DO
...
ELSE
  ...
  ! Retrieve old image properties
  imPositionBoxRotation    = imPositionMC
  imQuaternionBoxRotation  = imQuaternionMC
  imOrientationBoxRotation = imOrientationMC
  ! Retrieve old cylinder properties
  cPositionBoxRotation   = cPositionMC
  imcPositionBoxRotation = imcPositionMC
END IF
...
imPositionMC    = imPositionBoxRotation
imQuaternionMC  = imQuaternionBoxRotation
imOrientationMC = imOrientationBoxRotation

! Update cylinder properties
cPositionMC   = cPositionBoxRotation
imcPositionMC = imcPositionBoxRotation
